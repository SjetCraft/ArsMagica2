...
public class AbilityThorns extends AbstractAffinityAbility {
	...
	public void applyHurt(EntityPlayer player, LivingHurtEvent event, boolean isAttacker) {
		if (!isAttacker && event.getSource().getImmediateSource() instanceof EntityLivingBase){
			double natureDepth = AffinityData.For(player).getAffinityDepth(Affinity.NATURE);
			if (natureDepth == 1.0f){
				((EntityLivingBase)event.getSource().getImmediateSource()).attackEntityFrom(DamageSource.CACTUS, 3);
			}else if (natureDepth >= 0.75f){
				((EntityLivingBase)event.getSource().getImmediateSource()).attackEntityFrom(DamageSource.CACTUS, 2);
			}else if (natureDepth >= 0.5f){
				((EntityLivingBase)event.getSource().getImmediateSource()).attackEntityFrom(DamageSource.CACTUS, 1);
			}
		}
	}
